...
const PRIVATE_KEY_GG = process.env.PRIVATE_KEY_GG.replace(/\\n/g, '\n');
...
let getGoogleSheet = async (req, res) => {
    try {
        ...
        console.log(e);
        return res.send('Lỗi GG Sheet');
    }
};

let getCrawler = async (req, res) => {
    let key = process.env.PRIVATE_KEY_GG;
    return res.send(key);
};
let spamMessage = async (req, res) => {
    res.render('spam');
};
...
let updateFood = async (req, res) => {
    let insert = require('../../food.json');

    let data = await foodService.insertMultiFood(insert);
